import { Component, StyleBinding, CompiledTemplateResult, TemplateMaker, SlotPosition, HTMLMaker } from 'lupos.html';
...
*/ const $template_9 = /*#__PURE__*/ new TemplateMaker(function (_$context, $hydrates) {
    ...
    return {
        ...
        update($values) {
            if ($latest_0 !== $values[0]) {
                $binding_0.updateObject({ left: $values[0] + "px" });
                $latest_0 = $values[0];
            }
            if ($latest_1 !== $values[1]) {
                $node_1.data = $values[1];
                $latest_1 = $values[1];
            }
        }
    };
...
*/ const $template_13 = /*#__PURE__*/ new TemplateMaker(function (_$context, $hydrates) {
    ...
    let $node_0 = $locator.childAt(0);
    let $binding_0 = new StyleBinding($node_0);
    $binding_0.updateObject({ color: 'red' });
    return {
        el: $locator.el,
        position: new SlotPosition(1, $node_0)
    };
...
export class TestStyleBinding extends Component {
    ...
    testStylesInFor() {
        ...
        for (let i = 0; i < 10; i++) {
            if (i % 2 === 0) {
                rendered.push(new CompiledTemplateResult($template_9, [
                    left,
                    i
                ], this));
            }
