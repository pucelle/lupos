...
import {factory, Helper, Modifier, ts} from '../../../core'
	...
	preInit() {
		...
		for (let child of childNodes) {
			...
			if (valueIndex === null && child.tagName === 'lu:case') {
				this.slot.diagnoseNormal('<lu:case ${...}> must accept a parameter as condition!')
				break
			}

			if (valueIndex !== null && child.tagName === 'lu:default') {
				this.slot.diagnoseNormal('<lu:default> should not accept any parameter!')
				break
			}

			if (valueIndex === null && lastValueIndex === null) {
				this.slot.diagnoseNormal('<lu:default> is allowed only one to exist on the tail!')
				break
			}
