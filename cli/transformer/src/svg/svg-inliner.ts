...
export class SVGInliner {
	...
	private cleanSVGInner(svgInner: string) {
		...
		svgInner = svgInner.replace(/<!--[\s\S]*?-->|<title>[\s\S]*?<\/title>|<defs>[\s\S]*?<\/defs>|<desc>[\s\S]*?<\/desc>|<style.*?>[\s\S]*?<\/style>|enable-background:new\s*;|stroke-miterlimit:\s*\d+;/gi, '')
	...
	private processFillStroke(text: string) {
		...
		text = text.replace(/(<(?:path|circle|rect|ellipse|line|polyline|polygon))(.*?\/?>)/g, (tag, left, right) => {
			if (!/style="(.*?)"/.test(tag) && !/(stroke|fill)\s*=\s*"(#\w+)"/.test(tag)) {
